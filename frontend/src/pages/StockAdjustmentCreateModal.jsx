...
const StockAdjustmentCreateModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const fetchProducts = async () => {
        try {
            const response = await apiService.products.list({ limit: 1000 }); // simplified for now
    ...
    const fetchBatches = async (productId, warehouseId) => {
        ...
        try {
            ...
            const response = await apiService.stock.list({
                ...
                status: ['RECEIVED', 'ACTIVE', 'INSPECTED'], // Adjust based on available statuses
                limit: 100
            });
    ...
    const handleProductSelect = (product) => {
        ...
        fetchBatches(product.id, formData.warehouse_id);
        setNewItem({ ...newItem, batch_id: '', quantity_adjusted: '' });
    };

    const resetAddItemState = () => {
        setSelectedProduct(null);
        setAvailableBatches([]);
        setNewItem({ batch_id: '', quantity_adjusted: '', reason: '' });
        setProductSearch('');
        setIsAddingItem(false);
    };

    const handleAddItem = () => {
        if (!selectedProduct || !newItem.batch_id || !newItem.quantity_adjusted) {
        ...
        const item = {
            ...
            batch: batch, // Store full batch for display
            quantity_adjusted: parseFloat(newItem.quantity_adjusted),
            reason: newItem.reason || formData.reason,
            tempId: Date.now() // For UI key
        };
        ...
        try {
            ...
            const payload = {
                ...
                items: items.map(i => ({
                    ...
                    batch_id: i.batch_id,
                    quantity_adjusted: i.quantity_adjusted,
                    reason: i.reason
                }))
            };

            await apiService.stock.adjustments.create(payload);
                    ...
                    <div className="bg-card px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-text-secondary">Warehouse</label>
                                    <select
                                        required
                                        disabled={items.length > 0} // Lock warehouse if items added
                                    ...
                                    {items.length > 0 && <p className="text-xs text-text-secondary mt-1">Clear items to change warehouse</p>}
                                </div>
                            ...
                            <div className="border-t pt-4">
                                <div className="flex justify-between items-center mb-4">
                                    <h4 className="text-sm font-medium text-text-primary">Items to Adjust</h4>
                                    {!isAddingItem && (
                                        <button
                                            type="button"
                                            onClick={() => {
                                        ...
                                        <div className="space-y-3">
                                                            ...
                                                            >
                                                                ...
                                                                {availableBatches.map(b => (
                                                                    <option key={b.id} value={b.id}>
                                                                        #{b.id} - Qty: {b.available_qty} (Exp: {b.expiry_date ? new Date(b.expiry_date).toLocaleDateString() : 'N/A'})
                                                                    </option>
                                                                ))}
                                                            </select>
                                                        </div>
                                                        <div>
                                                            <label className="block text-xs font-medium text-text-secondary uppercase">Adjustment (+/-)</label>
                                                            <input
                                                                type="number"
                                                                step="0.01"
                                                                placeholder="-5 or 10"
                                                                value={newItem.quantity_adjusted}
                                                                onChange={(e) => setNewItem({ ...newItem, quantity_adjusted: e.target.value })}
                                                                className="mt-1 block w-full sm:text-sm border-gray-300 rounded-md"
                                                            />
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="flex justify-end space-x-2 pt-2">
                                                ...
                                                <button
                                                    ...
                                                    disabled={!selectedProduct || !newItem.batch_id || !newItem.quantity_adjusted}
                                ...
                                {items.length > 0 ? (
                                        ...
                                        <table className="min-w-full divide-y divide-theme-border">
                                            <thead className="bg-background">
                                                <tr>
                                                    ...
                                                    <th className="px-4 py-2 text-right text-xs font-medium text-text-secondary uppercase">Adjustment</th>
                                            ...
                                            <tbody className="divide-y divide-theme-border bg-card">
                                                {items.map((item) => (
                                                    <tr key={item.tempId}>
                                                        ...
                                                        <td className={`px-4 py-2 text-sm text-right font-medium ${item.quantity_adjusted > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                    ...
                                    !isAddingItem && (
                                        <div className="text-center py-8 bg-background rounded-lg border border-dashed border-gray-300">
                                            <p className="text-sm text-text-secondary">No items added to this adjustment.</p>
                                        </div>
                                    )
                                )}
                            </div>
                            ...
                            <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                                ...
                                >
                                    {loading ? 'Submitting...' : 'Create Adjustment'}
                                </button>
                                <button
                                    type="button"
                                    onClick={onClose}
